...
class PayementController extends AbstractController
{
    ...
    public function new(Request $request, PayementRepository $payementRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $payementRepository->add($payement);
    ...
    public function delete(Request $request, Payement $payement, PayementRepository $payementRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$payement->getId(), $request->request->get('_token'))) {
            $payementRepository->remove($payement);
        }

        return $this->redirectToRoute('app_payement_index', [], Response::HTTP_SEE_OTHER);
    }

    #[Route('/{id}/etudiants', name: 'app_payement_etudiant', methods: ['GET'])]
    public function payment(Payement $payement, EtudiantRepository $etudiantRepository)
    {
        $groupe = $payement->getGroupe();
        $etudiants = $etudiantRepository->findEtudiant($groupe);

        return $this->render('payement/etudiant.html.twig', [
            'etudiants' =>$etudiants,
        ]);
    }
}
